...
async def setup_local_repo(local_path: str, repo_name: str, repo_url_http: str, round_index: int) -> git.Repo:
    ...
    async with httpx.AsyncClient(timeout=45) as client:
        try:
            ...
            if round_index == 1:
                ...
                # Set origin to the PUBLIC (token-free) URL
                public_remote = repo_url_http + ".git"   # https://github.com/user/repo.git
                repo.create_remote('origin', public_remote)
                print("   -> R1: Local git repository initialized.")
            
            elif round_index >= 2:
                public_remote = repo_url_http + ".git"
                ...
                try:
                    repo = git.Repo.clone_from(public_remote, local_path)
                    print(f"   -> R{round_index}: Public clone succeeded.")
                except git.GitCommandError:
                    # Fallback to authenticated clone only for this operation
                    print(f"   -> R{round_index}: Public clone failed; attempting authenticated clone...")
                    temp_clone_url = f"https://{github_username}:{github_token}@github.com/{github_username}/{repo_name}.git"
                    repo = git.Repo.clone_from(temp_clone_url, local_path)
                    # Immediately sanitize origin so token is not persisted
                    try:
                        repo.remotes.origin.set_url(public_remote)
                    except Exception:
                        pass
                    print(f"   -> R{round_index}: Authenticated clone succeeded and origin sanitized.")
...
async def commit_and_publish(repo: git.Repo, task_id: str, round_index: int, repo_name: str) -> dict:
    ...
    async with httpx.AsyncClient(timeout=45) as client:
        try:
            ...
            # Replace direct push with a temporary-remote push so tokens are never persisted
            temp_remote_name = "deploy_token_remote"
            temp_remote_url = f"https://{github_username}:{github_token}@github.com/{github_username}/{repo_name}.git"

            # cleanup any existing temp remote object if present
            existing_remotes = {r.name: r for r in repo.remotes}
            if temp_remote_name in existing_remotes:
                try:
                    repo.delete_remote(existing_remotes[temp_remote_name])
                except Exception:
                    # best-effort
                    pass

            try:
                repo.create_remote(temp_remote_name, temp_remote_url)
                # push using the remote object (this sets remote branch)
                repo.git.push(temp_remote_name, 'main', force=True, set_upstream=True)
                print("   -> Changes pushed via temporary remote.")
            finally:
                # delete the temp remote object (best-effort)
                try:
                    if temp_remote_name in [r.name for r in repo.remotes]:
                        repo.delete_remote(repo.remote(temp_remote_name))
                except Exception as e:
                    print(f"   -> Warning: failed to delete temp remote: {e}")

            # make sure origin is token-free afterward
            try:
                origin_url = repo_url_http + ".git"
...
async def save_generated_files_locally(task_dir: str, files: dict) -> str:
    """
...
async def call_llm_for_code(prompt: str, task_id: str, image_parts: list) -> dict:
    ...
    headers = {
        "Authorization": f"Bearer {GEMINI_API_KEY}",    # Use Bearer (preferred)
...
async def save_attachments_locally(task_dir: str, attachments: list) -> list:
    ...
    for attachment in attachments:
        ...
        if not filename or not data_uri or not str(data_uri).startswith("data:"):
...
async def generate_files_and_deploy(task_data: TaskRequest):
    ...
    attachments = getattr(task_data, "attachments", None) or []
    ...
    try:
        ...
        if not repo:
            raise Exception("Failed to prepare local repository")
        ...
        for attachment in attachments:
            ...
            if not name or not url:
                print(f"   -> Skipping invalid attachment: {attachment}")
                continue

            if is_image_data_uri(url):
                gemini_part = data_uri_to_gemini_part(url)
                if gemini_part:
                    image_parts.append(gemini_part)

            attachment_list_for_llm_prompt.append(name)
        ...
        # save into the repo working dir so commit picks up the files
        await save_generated_files_locally(local_path, generated_files)
